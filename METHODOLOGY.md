# The Methodology of Integral Knowledge Architecture

*Tradition-neutral methodology for organizing, maintaining, and transmitting integral knowledge through AI. Thirteen architectural patterns, each forged through the construction of [Harmonism](https://harmonism.io) — a working ~470-file knowledge system with fractal structure, AI-augmented writing and translation pipelines, automated integrity checking, and a Companion intelligence faithful to its doctrine.*

*This document is the methodology in its current state of articulation. The reference implementations of individual patterns live in [`components/`](./components/) of this repository. The patterns themselves are summarised in [`patterns/`](./patterns/) with adoption notes per pattern.*

*Licensed CC-BY-4.0. Adopt freely. Attribution: Harmonia, https://harmonism.io.*
...
What is missing is not content. What is missing is *architecture* — a methodology for organising integral knowledge so that it can be navigated by human practitioners, taught by AI companions, maintained across languages, validated against its own standards, and extended without losing coherence. This document articulates that methodology as it has been developed through the construction of [[Harmonism]] — a 430-file interconnected knowledge system with fractal structure, AI-augmented writing and translation pipelines, automated integrity checking, and a Companion intelligence that learns from the corpus while remaining faithful to its doctrine.
...
Taxonomies murder integration. A library classification system ([Dewey](https://grokipedia.com/page/Dewey_Decimal_Classification), [Library of Congress](https://grokipedia.com/page/Library_of_Congress_Classification)) places each book in exactly one location, severing the connections that make integral knowledge integral. Tag-based systems (wikis, [Zettelkasten](https://grokipedia.com/page/Zettelkasten)) preserve connections but provide no architecture — the navigator drowns in a sea of equally weighted nodes with no sense of what is foundational, what is derived, and how the whole holds together. Hierarchical trees (academic departments, corporate org charts) impose false subordination — is psychology under biology or philosophy? The question itself reveals the architecture's inadequacy.
...
Any proposed element (pillar, spoke, sub-spoke) must satisfy three criteria drawn from [psychometric](https://grokipedia.com/page/Psychometrics) science:
...
Hierarchical models ([Maslow's pyramid](https://grokipedia.com/page/Maslow%27s_hierarchy_of_needs), where "lower" needs must be met before "higher" ones), dualistic models (sacred versus secular, theory versus practice), and flat-circle models that pretend all domains demand equal operational investment. The centre-spoke topology preserves both ontological co-equality (all spokes are real and irreducible) and operational asymmetry (the centre and certain spokes demand more investment than others, and investment in the centre pays dividends everywhere).
...
## V. The Companion as Transmission Architecture
...
This is categorically different from an AI tutor or a knowledge-base chatbot. An AI tutor teaches content; the Companion guides a *journey through an architecture*. The distinction matters because integral knowledge is not a body of information to be absorbed sequentially — it is a living structure to be inhabited, and the order in which someone encounters its parts determines whether the whole becomes legible. A person who encounters Harmonism through a Health protocol and then discovers the Presence dimension behind it has a fundamentally different relationship to the system than someone who reads the metaphysics first and tries to apply it afterward. The Companion knows this because the sequencing logic is encoded in its architecture — the content priority tiers, the Way of Harmony spiral, the alchemical sequence of preparing the vessel before filling it with light.

The guidance model is *self-liquidating*: the Companion's purpose is to teach people to read and navigate the architecture themselves, then step back. Success means the practitioner no longer needs the Companion — they have internalised the Wheel and can navigate it independently. This is the opposite of the engagement-maximisation logic that governs most AI products. The Companion's metric is not session length or return visits but the practitioner's growing capacity to orient themselves within the architecture without assistance.

Three capabilities distinguish the architectural companion from a generic AI assistant. First, *developmental tracking*: the Companion maintains a persistent Wheel-structured profile for each user, mapping their engagement across all pillars on a seven-point developmental scale and automatically determining their Way of Harmony phase. It knows not just what the person asked today but where they are in their long-term journey. Second, *sequenced guidance*: the Companion applies the system's own sequencing heuristics — ground in Health before ascending to Presence, don't skip structural phases, recognise when someone is in the Crucible of Relationships — rather than responding to queries in isolation. Third, *doctrinal fidelity*: the Companion speaks from within the system's philosophical ground rather than surveying it from outside, presenting settled doctrine with confidence and crystallising ideas with appropriate hedging.
...
**Terminological Discipline.** A knowledge system's technical vocabulary drifts into colloquial interpretation inside the AI companion. When a system uses "Service" to mean vocational alignment with [[Glossary of Terms#Dharma|Dharma]] and the model interprets it as the English word "service" (helping others, volunteering), the entire routing logic breaks. The solution is an explicit terminological attribution rule that maps each system term to its architectural meaning, overriding the model's natural-language intuitions. The transferable principle: any system whose vocabulary overlaps with everyday language — which is most systems — needs a terminological guard in its AI interface.
...
Five reference instantiations operationalize the pattern in Harmonia's deployment.

*The Living Book.* HTML book volumes regenerated from vault articles. Smart incremental update means a single article edit triggers only that chapter's regeneration; the rest of the book's HTML stays byte-identical. Reader-state (current page, bookmarks) survives regeneration because identifiers are stable across rebuilds. Six live volumes across ten languages — Philosophical Foundations, The Diagnosis, The Invisible Architecture, The Convergence, The Blueprint, The Horizon — with the broader Library of Harmonia architecture mapping 1:1 with Harmonism's registers.

*The Living Podcast.* Single-voice TTS canonical-article audio feed. Hash-per-section manifest records the SHA-256 of each section's source text at the time of generation. On regeneration, sections whose hash has not changed are skipped; sections whose hash has drifted are re-synthesized via the TTS provider and re-spliced into the master MP3. Cost reduction: editing one paragraph in a 6,000-word article regenerates approximately 30 seconds of audio instead of 25 minutes. Each episode corresponds one-to-one with a canonical article; when the article changes, the episode regenerates underneath the listener.

*The Living Video.* TTS audio (Layer 1, sourced from the Living Podcast pipeline) plus visual overlay (Layer 2 — hand-curated SVG asset palette over which AI orchestration assembles per-section visual presentation) plus orchestration (Layer 3 — pairs assets to audio timecodes, renders MP4). Regeneration follows the audio: when an article changes, only the affected audio sections regenerate, the visual orchestration re-runs only against those sections, and the final MP4 splices the new sections into the existing video file. No talking head, no founder face — the regeneration discipline is incompatible with footage of a specific person at a specific time.

*The Living Papers.* Academic-paper-register articles maintained as living documents in the vault, with date-stamped snapshots frozen for journal submission. The journal-submitted version is a fork preserved at a DOI; the living version continues to mature in the vault. When a reader queries the canonical URL, they receive the current version; when a reviewer queries the DOI, they receive the frozen snapshot. The two access paths are different by design — academic citation requires stable references, practitioner reading requires current doctrine, and the architecture serves both without forcing a choice.

*The Living System.* The vault's own meta-documentation — the description of how the system operates — regenerates from the actual operational state rather than being maintained as a separate document. The persistent-orientation file from Pattern IX is the most general instance: it describes the system as it currently is, and is updated at the end of every session that discovers something new about the system's behavior.

### Why It Works

The pattern resolves the publication-freeze problem at the architectural rather than the editorial level. Editors do not have to remember to update the audio when they update the article; the regeneration pipeline does it deterministically. The cost-per-edit collapses from the cost of full re-recording to the cost of the edited section, which is what makes the pattern operationally viable rather than aspirational.

The hash-manifest discipline is what prevents the failure mode of *paranoid regeneration* — where the pipeline regenerates everything on every edit because it cannot tell what changed. The manifest is the memory; without it, the pipeline is amnesiac and the economic case collapses. The discipline transfers to any output format with sectional structure: text artifacts split by heading, audio split by section break, video split by segment, with the same hash-comparison logic governing each.

The architectural commitment to *no talking head* in video is structurally entailed by the regeneration discipline, not added as a stylistic preference. Footage of a specific person at a specific time cannot be regenerated when doctrine evolves; doctrinal currency requires that the visual layer be assembled from regeneratable primitives. The asset-palette discipline (hand-curated SVG library plus AI orchestration over it, with no AI-generated assets) preserves authorial control over the visual identity while allowing the orchestration to track the audio's edits.

### What It Replaces

Versioned editions, errata pages, the institutional acceptance that artifacts drift from current doctrine. Continuous canonical regeneration makes the artifact's currency a property of the architecture rather than a virtue of editorial diligence. It also replaces the false choice between *publishing before doctrine is final* and *refusing to publish at all* — the canonical surface stays current with the doctrine because the artifacts regenerate as the doctrine evolves, and the snapshot mechanism preserves stable references for the contexts (academic citation, legal record) that genuinely require them.
...
The standard institutional response is *folders* — a research directory, a reading-notes subfolder, a tag soup of inbound material that grows faster than it gets integrated. The reading notes accumulate; the integration never happens; the system's actual doctrine remains uninfluenced by the content the operator has spent years feeding it. The folder becomes a graveyard of intentions.

### The Solution Pattern: The Six-Step Extraction Protocol

External content enters the system through a deterministic pipeline with six steps.

**1. Extract.** Convert source to clean Markdown with preserved metadata — author, source URL, publication date, citations, timestamps if audio or video. Marker or Docling or MinerU for PDFs; Whisper for audio and video; defuddle or similar for web articles. The extraction layer is a solved problem; pick the tool that fits the source and let it run. The output is a Markdown file with frontmatter encoding provenance.

**2. Assess.** Read the extracted content against the tradition's doctrinal architecture. What does this source claim? Where do its claims converge with the tradition's existing canon? Where do they diverge? What kernels does it contain that the tradition has not yet articulated? This step requires doctrine-fluent intelligence — operator or AI companion with the doctrinal backbone in context — and is the step the protocol does not attempt to fully automate.

**3. Identify kernels.** Extract the specific claims, frameworks, or observations the tradition might integrate. A kernel is a discrete unit of intellectual content that can be moved from the source into the tradition's architecture without losing meaning. Most source material yields zero to three kernels; a few yield more. The discipline: a kernel is not a quote, and capturing a quote without identifying its kernel is the failure mode that turns the protocol into a highlights collection.

**4. Reframe in native language.** Translate each kernel from the source's vocabulary into the tradition's terminology. *Mindfulness* in the source becomes *Presence — sati in the Pāli* in the Harmonist register. The reframing is doctrinal work, not editorial — the kernel's meaning must survive translation into the tradition's frame, or the kernel was not what it appeared to be. When a kernel does not survive reframing, the protocol returns to step 2 and the source's content is re-assessed; sometimes the kernel was real but the tradition's current articulation does not yet have language for it, in which case step 4 surfaces a vocabulary gap that becomes its own editorial work.

**5. Route to vault location.** Place each reframed kernel where it belongs in the tradition's architecture. New content seeds new bridge articles; existing canon receives the kernel as a citation or expansion. Pattern III's routing rule applies absolutely: route to `bridge` or `applied`, never to `canon`. If no appropriate bridge article exists, seed one rather than contaminating canon; the seeded article begins at `breadth: partial`, `depth: introductory`, `craft: muddy`, and matures through normal editorial cycles.

**6. Verify.** Read the destination article(s) after integration. Does the kernel land coherently? Does it strengthen the article's claims, or create internal contradiction? If the latter, the kernel needs editorial work before integration, or the integration was misrouted. The verification step is what prevents accumulation of kernels that were technically routed but functionally orphaned.
...
Two sibling pipelines specialize the protocol for input formats. The *audio/video pipeline* takes an audio file or video URL through Whisper transcription to cleaned Markdown with preserved timestamps, then through steps 2 through 6. The timestamp preservation is what makes the output queryable — a kernel routed into a vault article carries a reference back to the source audio at the specific moment the operator can revisit. The *web/PDF pipeline* takes a URL or PDF path through defuddle or Marker to cleaned Markdown with preserved metadata, then through the same steps 2 through 6. The metadata preservation is what makes the output citable — the routed kernel carries its source attribution forward across every downstream use.

The protocol's discipline against *capture-without-integration* is structural rather than aspirational. Step 1 alone is what produces highlights collections; the protocol's value is in the four steps that follow. An operator who runs step 1 and skips the rest is not using the protocol; they are using an extraction tool, which solves a different problem.

### What It Replaces

The folder-based inbox model that accumulates faster than it gets integrated. The tag-based knowledge-management model that confuses *capture* with *integration*. The reading-notes application — the highlight-collection tool that produces annotated lists without ever routing them into a doctrinal architecture — that has produced more orphaned highlights than any other category of knowledge software. The six-step extraction protocol makes external-content integration a deterministic process rather than an editorial aspiration.

---

*See also: [[Harmonism]], [[Anatomy of the Wheel]], [[MunAI]], [[Glossary of Terms]]*
